...
{
	insert_head_in_linkedlist(&(bucket->data_list), new_data);
}
...
void deinit_bucket(c_bucket* bucket)
{
	...
	{
		c_data* data_to_delete = (c_data*) get_head_of_linkedlist(&(bucket->data_list));
		remove_from_linkedlist(&(bucket->data_list), data_to_delete);
		deallocate_data(data_to_delete->data_class, data_to_delete);
	}
}